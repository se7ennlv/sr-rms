...
try {
    ...
    dbo.Questions.QuesEnName + ' / ' +  dbo.Questions.QuesThName AS [Questions],
    SUM(dbo.SpaSurveyDetails.Rating) AS Rating,
    COUNT (*) AS [NoOfScorer],
        CAST (
            CAST (
                SUM (dbo.SpaSurveyDetails.Rating) AS DECIMAL
    ...
    WHERE SpaSurveyDetails.CreatedAt BETWEEN '{$fromDate}' AND '{$toDate}'
    GROUP BY EmpName,DeptName,QuesEnName,QuesThName";
    ...
    // $sql3 = "SELECT COUNT ( dbo.SpaSurveyDetails.Rating ) AS RatingStar FROM SpaSurveyDetails WHERE ( SpaSurveyDetails.CreatedAt >= DATEADD(D, - 30, GETDATE()) AND SpaSurveyDetails.CreatedAt < GETDATE() )";
