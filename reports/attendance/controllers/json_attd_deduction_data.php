...
try {
    ...
    $sql = "WITH cte AS (SELECT TOP(100)PERCENT 
            ...
            WHERE (attd.ShiftCode LIKE 'C%' OR attd.ShiftCode = 'AB') AND attd.EmpCode NOT IN (SELECT ExemID FROM NORMEXT.dbo.tbl_exempted) 
            )
            ...
            FROM cte
            WHERE (cte.WorkDay BETWEEN '{$fromDate}' AND '{$toDate}') AND cte.EmpStatus = 1 AND cte.ShiftCode LIKE 'C%' AND (cte.LateCheckIn + cte.EarlyCheckOut) > 0
            ";
